...
const WS_ONOFF = process.env.WS_ONOFF || 1;

async function wss() {
  ws = new WebSocket('wss://iboard-pushstream.ssi.com.vn/realtime');
  if (WS_ONOFF == 1) {
    console.log('Init to ws server ', 'wss://iboard-pushstream.ssi.com.vn/realtime', WS_ONOFF, process.env.WS_ONOFF)
    ws.on('open', function () {
...
class MessageReader extends Model {
  async reader() {
    ...
    READ_DATE = filename.slice(-12).slice(0, 8)
    console.log('READ_DATE', READ_DATE)
...
class PriceModel {
  ...
  async onLetable(message) {
    ...
    let timeX;
    if (READ_DATE) {
      timeX = moment(READ_DATE + time, "YYYYMMDDHH:mm").format("X")
    } else {
      timeX = moment(time, "HH:mm").format("X")
    }
    ...
    //   console.table(stats)
    let table = {
      labels: ["symbol", "time", "T", "price", "change", "pct", "bid_vol", "bid_val", "ask_vol", "ask_val", "bu_vol", "bu_val", "sd_vol", "sd_val", "unknown_vol", "unknown_val", "busd_vol", "busd_val",],
    }
    table.data = table.labels.map(e =>
      stats[e]
    )
