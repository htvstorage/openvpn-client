...
//https://static2.vietstock.vn/vietstock/2023/6/15/20230615_16062023_ho.pdf
...
function url(date) {
    ...
    let file = t + '_' + t1 + '_ho.pdf';
    const url = 'https://static2.vietstock.vn/vietstock/' + date.getFullYear() + '/' + (date.getMonth() + 1) + '/' + date.getDate() + '/' + file;
    return { url, file };
}

//https://static2.vietstock.vn/vietstock/2023/6/14/1_15062023_hnx.pdf

function urlHNX(date) {
    // let t = date.getFullYear() + ""
    //     + (date.getMonth() + 1 < 10 ? ("0" + (date.getMonth() + 1)) : date.getMonth() + 1) + ""
    //     + (date.getDate() < 10 ? "0" + date.getDate() : date.getDate())
    let t =
...
(async () => {
    ...
    for (var i = 1; i < 10; i++) {
        let x = urlHNX(new Date(Date.now() - i * 24 * 60 * 60 * 1000));
        const response = fetch(x.url);
        req++;
        response.then(rs => rs.buffer()).then(async (buffer) => {
            if (buffer.length > 500) {
...
(async () => {
    ...
    for (var i = 0; i < 200; i++) {
        let xa = date2str(new Date(Date.now() - i * 24 * 60 * 60 * 1000));
        console.log(xa)
        xa.forEach(x=>{
    ...
    console.log("Files",fileFiltered)
    files = fileFiltered;
    for (let file of files) {
