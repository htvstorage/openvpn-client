...
(async () => {
  let cop = [{ stock_code: 'VNINDEX', post_to: 'HOSE' }, { stock_code: 'VN30', post_to: 'HOSE' }];
  let data = null;
...
async function getPrices(symbol) {
  ...
  let f = async (symbol) => {
    return await fetch("https://restv2.fireant.vn/symbols/" + symbol + "/historical-quotes?startDate=2000-12-06&endDate=" + getNow() + "&offset=0&limit=20000000", {
      "headers": {
        "accept": "application/json, text/plain, */*",
        "accept-language": "en-US,en;q=0.9,vi-VN;q=0.8,vi;q=0.7",
